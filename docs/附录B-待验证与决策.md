...
49. opus-mt zh↔en int8 在基线机（iPhone 12 / 骁龙 7 系）与旗舰机的单句延迟 P50 / P95（10 / 20 / 40 token）、常驻内存、冷启动加载时间；与 zipformer + 系统 TTS 同跑 10 分钟的发热与降频曲线（周 4 spike）。 **2026-09-17 补**：Xenova/opus-mt-zh-en、en-zh 的 int8 ONNX（encoder 52.7 MB + decoder_merged 60.0 MB + spm/vocab）每方向 ≈ 115 MB（文档"≈ 80 MB"偏低）；解码 I/O 名与 generation_config 已核实（decoder_start = pad = 65000、eos = 0、en→zh 句首加 `>>cmn_Hans<<`）；zh-en 基模型 CC-BY-4.0、en-zh Apache-2.0。实现后移为 I3.5（13 篇决策记录）。 **2026-09-18 落地**：vivo V2054A（骁龙 480）中 → 英 11 token 462 ms、英 → 中 18 token 329 ms，装载 ≈ 1.4 s（首次多 1 s 改写）；两方向常驻 352 MB；仅听会话 PSS 745 MB。Xenova 导出的 lm_head（共享词嵌入）每步 `DequantizeLinear` 133 MB float 是内存 / 时延主因，已在下载后本机改写成 `MatMulInteger`（`NmtModelPatch`）。基线机 iPhone 12 / 骁龙 7 系数据、beam 2–4 的收益、与 ASR + TTS 同跑发热仍待测。
50. opus-mt 模型许可（Helsinki-NLP 标 CC-BY-4.0，署名义务与商用条款待法务核实）；SentencePiece 模型随包分发的许可；int8 量化后与 fp32 的 BLEU / 人工可懂度差异。 **2026-09-18 补**：HF cardData 核实只有 zh-en 是 CC-BY-4.0，en-zh / ja-en / ko-en / en-mul / mul-en 均 Apache-2.0。贪心解码下 en-zh 对英文短句会续写（"The day after tomorrow." → "后天，我再给你打电话"），需评估 beam 或长度惩罚能否压住。
51. ONNX Runtime 与 sherpa-onnx 共用同一份运行时的链接冲突（静态库符号重复、版本锁定、iOS xcframework 与 Android AAR 各自的单副本策略）；不可解时的隔离方案。 **2026-09-17 补**：Android 已证实——Maven Central 无 onnxruntime-android 1.28.2（只有 1.28.0 / 1.29.0 / 1.30.0），sherpa 1.13.8 的 libonnxruntime.so 只导出 `OrtGetApiBase@@VERS_1.28.2`，bionic 要求符号版本精确匹配，Maven 1.28.0 的 JNI 库必炸；可行路径 = Maven 1.28.0 的 classes.jar + csukuangfj onnxruntime-android-1.28.2.zip 内的 libonnxruntime4j_jni.so + sherpa AAR 的 libonnxruntime.so（同版本，不需 pickFirst）。iOS：sherpa 的 onnxruntime.xcframework 含 Headers，K/N 2.4.10 对 onnxruntime_c_api.h 的 cinterop 已试通，可与 sherpa.def 并存（不要重复 staticLibraries）。 **2026-09-18 落地**：csukuangfj 的 `onnxruntime-android-1.28.2.zip` 本身就是 AAR 布局（classes.jar + JNI + headers），直接当 AAR 用，app 侧 `pickFirst libonnxruntime.so`（与 sherpa AAR 内的 sha256 相同）；iOS cinterop 只导头文件（`nonStrictEnums` 列出用到的枚举），模拟器单测通过。
...
54. 更多 opus-mt 语言对（en↔ja / en↔ko / 欧语）的可用模型与质量（v1.1 评估）。 **2026-09-18 落地**：en↔ja 用 FuguMT（staka，CC-BY-SA-4.0；Kadonox 的 optimum int8 导出），opus-mt-en-mul 的日语不能用；en→ko 没有可用模型（opus-mt-tc-big-en-ko 的 HF 转换缺源词表，英文全成 <unk>；三家 ONNX 导出都继承了这个坏词表），只能等上游修或自己从 Marian 原始模型转；ko→en 用 opus-mt-ko-en。欧语（en↔de/fr/es…）Xenova 都有导出，本项目语言集之外，需要时按同样五文件加进目录即可。
