...
- **2026-09-18 · 用户反馈的问题与优化清单**
  - 新增 [问题与优化清单](问题与优化清单.md)：2026-09-17 ～ 09-18 两天里由用户在真机 / 模拟器上提出的 **27 条 Bug** 与 **19 条优化**，逐条记原话、根因、改法、验证进度、状态与对应提交；末尾附「我排查时附带修掉的」6 条和「还欠着的」8 项。
  - 其中 2 条查下来不成立（双屏崩溃是反复重装撞上的误判；Gradle 同步报错是 IDE 的 Android 插件跟不上 AGP 9.1.1），1 条修完按用户要求回滚（外放期间的麦克风门控，代价是外放只能半双工）。
